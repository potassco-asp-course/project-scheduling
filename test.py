...
def call_clingo(input_names, timeout, expected):
    if expected == "SAT":
...
def test(enc, solutions_dir, inst, timeout, expected):
    # check if encoding result is as expected
    try:
        if expected == 'SAT':
            stdout, time = call_clingo([enc, INSTANCES+inst, SOLUS], timeout, expected)
        else:
            stdout, time = call_clingo([enc, INSTANCES+inst, DUMMY], timeout, expected)
    ...
    if expected == 'SAT':
        ...
        with open(solutions_dir+"/"+inst_sol,"r") as infile:
...
def main():
    # check input
    if len(sys.argv) < 5:
        raise RuntimeError("not enough arguments (%d instead of 4)" % (len(sys.argv) - 1))
    enc, solutions_dir, timeout, expected = sys.argv[1:5]
    timeout = int(timeout)
    for f in [enc]:
    ...
    for inst in dir:
        ...
        error = False
        try:
            res, time = test(enc, solutions_dir, inst, timeout, expected)
            if not res:
                success = False
